<script>
  ...
  let selectedArea = config.sta && config.sta.includes('-') ? config.sta.split('-')[0] : ''; // Only set from config if it exists, otherwise empty
  let selectedLine = config.sta.split('-')[1] || '';
  let selectedDirection = '';
  let selectedStopId = config.sta && config.sta.includes('-') ? config.sta.split('-')[2] || '' : ''; // The actual stop ID from the config
  ...
  $: if (selectedArea && selectedLine && selectedStopId) {
    ...
    console.log('Reactive: Dispatching config change:', newSta);
    ...
    console.log('Reactive: Dispatching empty config');
  ...
  async function loadRouteDirections(area, routeCode) {
    ...
    try {
      ...
      const routeDetails = await dataService.loadGmbRouteDetails(area, routeCode);
      ...
      if (Array.isArray(routeDetails) && routeDetails.length > 0) {
        ...
        if (routeInfo && routeInfo.directions) {
          ...
          gmbDirections = [
            { route_seq: 1, dest_en: `${routeCode} - Outbound`, dest_tc: `${routeCode} - 去程` },
            { route_seq: 2, dest_en: `${routeCode} - Inbound`, dest_tc: `${routeCode} - 返程` }
          ];
        }
      } else {
        gmbDirections = [];
  ...
  async function loadRouteStops(directionSeq) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch GMB route stops: ${response.status} ${response.statusText}`);
      }
  ...
  async function handleDirectionChange(event) {
    ...
    if (newDirection && cachedRouteId) {
      console.log('Loading stops for direction:', newDirection, 'with route ID:', cachedRouteId);
      await loadRouteStops(newDirection);
      console.log('Loaded stops:', gmbStops.length);
    }
...
<div class="gmb-config-form space-y-4">
  ...
  <!-- Direction Selection -->
  <div class="mb-4 grid grid-cols-2 gap-4 items-center">
    <label for="gmb-direction" class="w-full text-left">
      Direction:
    </label>
  ...
  <!-- Stop Selection -->
  <div class="mb-4 grid grid-cols-2 gap-4 items-center">
    <label for="gmb-stop" class="w-full text-left">
      Stop:
    </label>
    ...
    {:else}
      <input
        ...
        id="gmb-stop-id"
        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 font-mono"
        placeholder={selectedDirection ? "Loading stops..." : "Select a direction first"}
        value=""
        disabled={true}
      />
    {/if}
