<script>
  ...
  ? mtrbusRoutesData.map(route => route.route_number).sort((a, b) => 
  ...
  $: lines = selectedRouteObject && selectedRouteObject.lines
  ...
  $: {
    ...
    if (lines.length > 0 && selectedLineIndex !== "" && !lines[parseInt(selectedLineIndex)]) {
      selectedLineIndex = "0";
    }
  ...
  $: {
    ...
    const currentStopIsValid = selectedStopRefId && stops.some(s => s.ref_ID === selectedStopRefId);
  ...
  onMount(() => {
    if (config.sta && Array.isArray(mtrbusRoutesData)) {
      ...
      } else if (staParts.length === 1) { // e.g., "K51" (only route selected)
  ...
  <div class="mb-4 grid grid-cols-2 gap-4 items-center">
    ...
    >
      {#each lines as line, index}
        <option value={index.toString()} selected={index.toString() === selectedLineIndex}>{line.description_en}</option>
      {/each}
      {#if lines.length === 0}
        <option value="" disabled selected={selectedLineIndex === ""}>{!selectedRoute ? "Select a route first" : "No directions available"}</option>
      {/if}
    </select>
  </div>
