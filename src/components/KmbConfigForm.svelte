<script>
	...
	$: directions =
		selectedRoute && kmbRoutesData[selectedRoute]
			? kmbRoutesData[selectedRoute].map((routeData, index) => {
					const originText =
						routeData[`orig_${currentLanguage}`] || routeData.orig_en;
					const destinationText =
						routeData[`dest_${currentLanguage}`] || routeData.dest_en;
					return {
						value: index,
	...
	function getStopsForRoute(route, directionIndex) {
		if (
			...
			directionIndex === '' ||
			!kmbRoutesData[route] ||
			!kmbRoutesData[route][directionIndex]
		) {
			return [];
		}
		...
		const bound =
			routeVariant.bound === '1'
				? 'O'
				: routeVariant.bound === '2'
					? 'I'
					: routeVariant.bound;
			...
			if (staParts.length === 4) {
				...
				if (kmbRoutesData[route]) {
					const directionIndex = kmbRoutesData[route].findIndex(
						routeData =>
							(routeData.bound === '1' && bound === 'O') ||
							(routeData.bound === '2' && bound === 'I') ||
							(routeData.bound === bound &&
	...
	<div class="mb-4 grid grid-cols-2 gap-4 items-center">
		...
		>
			{#each directions as direction}
				<option value={direction.value} title={direction.title}
