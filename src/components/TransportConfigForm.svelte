<script>
	...
	async function handleProviderChange() {
		...
		if (
			...
			Object.keys($kmbRoutesData).length === 0
		) {
			loading.set(true);
			try {
				const kmbData = await dataService.loadKmbData();
			...
			$config.transportETAProvider === 'mtrbus' &&
			Object.keys($mtrbusRoutesData).length === 0
		) {
			loading.set(true);
			try {
				const mtrbusData = await dataService.loadMtrbusRoutesData();
			...
			Object.keys($ctbRoutesData).length === 0
		) {
			loading.set(true);
			try {
				const ctbData = await dataService.loadCtbData();
			...
			Object.keys($gmbRoutesData).length === 0
		) {
			loading.set(true);
			try {
				const gmbData = await dataService.loadGmbData();
			...
			Object.keys($lrtStationsData).length === 0
		) {
			loading.set(true);
			try {
				const lrtData = await dataService.loadLrtStationsData();
	...
	<form class="space-y-4">
		...
		<div class="mb-4 grid grid-cols-2 gap-4 items-center">
			...
			<select
				...
				on:change={e => {
					handleConfigChange('transportETAProvider', e.target.value);
			...
			<CtbConfigForm
				...
				on:routeSelected={e => {
					...
					if (selectedRoute) {
						loading.set(true);
						dataService
							.loadCtbRouteStopsData(selectedRoute)
							.then(data => ctbRouteStopsData.set(data))
							.catch(err =>
								error.set(`Failed to load CTB route stops data: ${err.message}`)
							)
			...
			<div class="space-y-4 border-t pt-4">
				...
				<div class="mb-4 grid grid-cols-2 gap-4 items-center">
					...
					<input
						...
						on:input={e =>
							handleConfigChange('reloadInterval', parseInt(e.target.value))}
						min="1"
						class:border-green-500={!isDifferentFromDefault(
							'reloadInterval',
							$config.reloadInterval
						)}
						class:border-blue-500={isDifferentFromDefault(
							'reloadInterval',
							$config.reloadInterval
						)}
					/>
				...
				<div class="mb-4 grid grid-cols-2 gap-4 items-center">
					<label for="animationSpeed" class="w-full text-left">
						{$translations.labelAnimationSpeed ||
					...
					<input
						...
						on:input={e =>
							handleConfigChange('animationSpeed', parseInt(e.target.value))}
						min="0"
						class:border-green-500={!isDifferentFromDefault(
							'animationSpeed',
				...
				<div class="mb-4 grid grid-cols-2 gap-4 items-center">
					<label for="initialLoadDelay" class="w-full text-left">
						{$translations.labelInitialLoadDelay ||
					...
					<input
						...
						on:input={e =>
							handleConfigChange('initialLoadDelay', parseInt(e.target.value))}
						min="0"
						class:border-green-500={!isDifferentFromDefault(
							'initialLoadDelay',
